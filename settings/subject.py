...
class mainWindow:
    def __init__(self):
        ...
        self._name = ''
        self.win = Toplevel()
        self.win.title('考试科目设置')
        self.tree = Treeview(self.win, columns=('ID', 'Name'), show='headings')
        self.tree.heading('ID', text='科目编号', anchor='center')
        self.tree.heading('Name', text='科目名称', anchor='center')
        self.tree.grid(row=0, column=0, columnspan=4)
        Label(self.win, text='科目编号').grid(row=1, column=0)
        Label(self.win, text='科目名称').grid(row=1, column=2)
    ...
    def add(self):
        if self.ID.get() == '' or self.name.get() == '':
            showwarning('提示', '请完善数据。')
                ...
                result = service.exec('insert into tb_subject(subID,subName) values (%s,%s)', data)
                if result == 1:
                    self.ID.delete(0, END)
                    self.name.delete(0, END)
                    self.query()
    ...
    def change(self):
        ...
        if self._ID == self.tree.set(self.tree.focus())['ID'] \
            ...
            service.exec('delete from tb_subject where subID = %s', self.tree.set(self.tree.focus())['ID'])
            result = service.exec('insert into tb_subject(subID,subName) values (%s,%s)', (self._ID, self._name))
            if result == 1:
                self.query()
                showinfo('提示', '修改数据成功。')
                self.ID.delete(0, END)
    ...
    def delete(self):
        if self.tree.focus() == '':
            ...
            result = service.exec('delete from tb_subject where subID = %s', self.tree.set(self.tree.focus())['ID'])
            if result == 1:
                self.name.delete(0, END)
                self.ID.delete(0, END)
                self.query()
